# Weakly/student_dwunet.py
# Student: lightweight depthwise-separable U-Net.
# Input:  [B, 1, H, W]
...
class DWDoubleConvIO(nn.Module):
    ...
    """
    def __init__(self, in_c, out_c, r=4):
        super().__init__()
...
class DWUNet(nn.Module):
    ...
    """
    def __init__(self, in_ch=1, base=32, out_ch=1, r=4):
        super().__init__()
        b = base
        ...
        # Encoder
        self.d1 = DWDoubleConvSame(b, r=r);       self.p1 = nn.MaxPool2d(2)
        self.enc2_proj = nn.Conv2d(b,   b*2, 1, bias=False)
        self.d2 = DWDoubleConvSame(b*2, r=r);     self.p2 = nn.MaxPool2d(2)
        self.enc3_proj = nn.Conv2d(b*2, b*4, 1, bias=False)
        self.d3 = DWDoubleConvSame(b*4, r=r);     self.p3 = nn.MaxPool2d(2)
        self.enc4_proj = nn.Conv2d(b*4, b*8, 1, bias=False)
        self.d4 = DWDoubleConvSame(b*8, r=r);     self.p4 = nn.MaxPool2d(2)
    ...
    def forward(self, x):
        d1s = self.stem(x)
        d1  = self.d1(d1s);   p1 = self.p1(d1)

        e2  = self.enc2_proj(p1)
        d2  = self.d2(e2);    p2 = self.p2(d2)

        e3  = self.enc3_proj(p2)
        d3  = self.d3(e3);    p3 = self.p3(d3)

        e4  = self.enc4_proj(p3)
        d4  = self.d4(e4);    p4 = self.p4(d4)

        bt_i = self.bt_proj(p4)
        bt   = self.bt(bt_i)

        x = self.u4(bt); x = self.c4(torch.cat([x, d4], 1))
...
def make_student_dwunet(in_ch=1, out_ch=1, base=32, r=4):
    return DWUNet(in_ch=in_ch, out_ch=out_ch, base=base, r=r)
